#ifndef STOPPROFITTRIGGER_H
#define STOPPROFITTRIGGER_H
#include"triggers.h"
using namespace std;
class stopprofitTrigger: public triggers{
public:
    stopprofitTrigger():triggers(){}
    stopprofitTrigger(cryptoCoin* _coin,double _price):triggers(_coin,_price){}
    stopprofitTrigger(const stopprofitTrigger& other):triggers(other){}
    ~stopprofitTrigger()override{}

    const String getTriggerType()const override{
        return "StopProfit Trigger";
    }
    ...
    void executeTriggerInfo()override{
        cout<<"[StopProfit Selling]: \""<<getTriggerCoin()->getSymbol()<<"\" sold at $"<<getTriggerCoin()->getPrice()<<
