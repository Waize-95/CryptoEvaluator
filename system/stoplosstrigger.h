#ifndef STOPLOSSTRIGGER_H
#define STOPLOSSTRIGGER_H
#include"triggers.h"
using namespace std;
class stoplossTrigger: public triggers{
public:
    stoplossTrigger():triggers(){}
    stoplossTrigger(cryptoCoin* _coin,double _price):triggers(_coin,_price){}
    stoplossTrigger(const stoplossTrigger& other):triggers(other){}
    ~stoplossTrigger()override{}

    const String getTriggerType()const override{
        return "StopLoss Trigger";
    }
    ...
    void executeTriggerInfo()override{
        cout<<"[StopLoss Selling]: \""<<getTriggerCoin()->getSymbol()<<"\" sold at $"<<getTriggerCoin()->getPrice()<<
